...
$post = $offerObject->getPostByID($_GET['id']);

if (($post['user_account_id'] == $_SESSION['logged_user_id']) || ($userPermission == '3')){


//Memory old values
$name = $post['name'];
$job_typeID = $post['job_type_id'];
$job_type_name = ucfirst($offerObject->getJobTypeByID($post['job_type_id']));
$job_provinceID = $post['job_province_id'];
$job_province_name = ucfirst($offerObject->getJobProvinceByID($post['job_province_id']));
...
if(isset($_POST['submit'])){
	...
	if ($validator->foundErrors()) {
		...
		if($offerObject->update($_POST, $post['id'])){
...
<div class="container pb-5">
	<div class="col-12 col-xs-6 col-xs-offset-3">
		...
		<h3 class="pt-3 pb-3">Edycja oferty</h3> 
		...
		<form method="POST">
		  ...
		  <div class="form-group">
		    ...
		    <select name="job_type" class="form-control" id="exampleFormControlSelect1">
		      <?php 
		      	echo '<option selected value="'.$job_typeID.'">'.$job_type_name.'</option>';
		...
		<div class="form-group">
		    ...
		    <?php 
		    	echo '<option selected value="'.$job_provinceID.'">'.$job_province_name.'</option>';
